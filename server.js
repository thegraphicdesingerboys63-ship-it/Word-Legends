...
async function initDb() {
  const stmts = [
    ...
    `CREATE TABLE IF NOT EXISTS notifications (
      ...
      type TEXT NOT NULL, message TEXT NOT NULL, read INTEGER DEFAULT 0,
    ...
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS active_card_bg TEXT DEFAULT 'default'`,
    `CREATE TABLE IF NOT EXISTS user_card_bgs (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id) ON DELETE CASCADE, bg_id TEXT NOT NULL, UNIQUE(user_id,bg_id))`,
    ...
    `CREATE TABLE IF NOT EXISTS forum_reactions (
      ...
      type TEXT NOT NULL, created_at TIMESTAMP DEFAULT NOW(),
    ...
    `CREATE TABLE IF NOT EXISTS game_ratings (
      ...
      comment TEXT, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW()
    )`,
    `CREATE TABLE IF NOT EXISTS user_achievements (
      id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
      achievement_id TEXT NOT NULL, unlocked_at TIMESTAMP DEFAULT NOW(),
      UNIQUE(user_id, achievement_id)
    )`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS is_muted INTEGER DEFAULT 0`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS bio TEXT DEFAULT NULL`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS daily_streak INTEGER DEFAULT 0`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS last_daily_date TEXT DEFAULT NULL`,
    `CREATE TABLE IF NOT EXISTS daily_challenge_completions (
      id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
      date_str TEXT NOT NULL, completed_at TIMESTAMP DEFAULT NOW(),
      score INTEGER DEFAULT 0, words_found INTEGER DEFAULT 0,
      UNIQUE(user_id, date_str)
    )`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS words_found_total INTEGER DEFAULT 0`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS bonus_found_total INTEGER DEFAULT 0`,
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS no_hint_levels INTEGER DEFAULT 0`,
...
// ── ACHIEVEMENTS ──────────────────────────────────────────────────────────────
const ACHIEVEMENTS = {
  first_level:    {id:'first_level',   name:'First Steps',      icon:'⭐',rarity:'common',   color:'#95a5a6',desc:'Complete your first level',            cat:'Levels'},
  explorer_10:    {id:'explorer_10',   name:'Explorer',         icon:'🗺️',rarity:'common',   color:'#2ecc71',desc:'Complete 10 levels',                   cat:'Levels'},
  veteran_50:     {id:'veteran_50',    name:'Veteran',          icon:'🎖️',rarity:'uncommon', color:'#27ae60',desc:'Complete 50 levels',                   cat:'Levels'},
  centurion_100:  {id:'centurion_100', name:'Centurion',        icon:'🏅',rarity:'rare',     color:'#2980b9',desc:'Complete 100 levels',                  cat:'Levels'},
  grind_500:      {id:'grind_500',     name:'Grinder',          icon:'📕',rarity:'epic',     color:'#8e44ad',desc:'Complete 500 levels',                  cat:'Levels'},
  legend_1000:    {id:'legend_1000',   name:'Legend',           icon:'🪄',rarity:'legendary',color:'#e74c3c',desc:'Complete 1,000 levels',                cat:'Levels'},
  words_100:      {id:'words_100',     name:'Word Finder',      icon:'🔤',rarity:'common',   color:'#95a5a6',desc:'Find 100 total words',                 cat:'Words'},
  words_500:      {id:'words_500',     name:'Wordsmith',        icon:'📚',rarity:'uncommon', color:'#f39c12',desc:'Find 500 total words',                 cat:'Words'},
  words_2500:     {id:'words_2500',    name:'Word Scholar',     icon:'🏛️',rarity:'rare',     color:'#9b59b6',desc:'Find 2,500 total words',               cat:'Words'},
  words_10k:      {id:'words_10k',     name:'Word Legend',      icon:'✨',rarity:'epic',     color:'#e74c3c',desc:'Find 10,000 total words',              cat:'Words'},
  bonus_first:    {id:'bonus_first',   name:'Lucky Find',       icon:'🍀',rarity:'common',   color:'#27ae60',desc:'Find your first bonus word',           cat:'Bonus'},
  bonus_hunter:   {id:'bonus_hunter',  name:'Bonus Hunter',     icon:'🌟',rarity:'uncommon', color:'#f1c40f',desc:'Find 10 bonus words',                  cat:'Bonus'},
  bonus_master:   {id:'bonus_master',  name:'Bonus Master',     icon:'💫',rarity:'rare',     color:'#9b59b6',desc:'Find 50 bonus words',                  cat:'Bonus'},
  no_hints_first: {id:'no_hints_first',name:'Clean Run',        icon:'🎯',rarity:'common',   color:'#2ecc71',desc:'Complete a level without hints',       cat:'Mastery'},
  no_hints_10:    {id:'no_hints_10',   name:'Sharp Mind',       icon:'💎',rarity:'rare',     color:'#00b4d8',desc:'Complete 10 levels without hints',     cat:'Mastery'},
  no_hints_50:    {id:'no_hints_50',   name:'Purist',           icon:'🔮',rarity:'epic',     color:'#8e44ad',desc:'Complete 50 levels without hints',     cat:'Mastery'},
  speedster:      {id:'speedster',     name:'Speedster',        icon:'⚡',rarity:'uncommon', color:'#e67e22',desc:'Complete a level in under 90 seconds',  cat:'Mastery'},
  friend_first:   {id:'friend_first',  name:'Making Friends',   icon:'🤝',rarity:'common',   color:'#3498db',desc:'Add your first friend',                cat:'Social'},
  social_5:       {id:'social_5',      name:'Social Butterfly', icon:'👥',rarity:'uncommon', color:'#2980b9',desc:'Have 5 friends',                       cat:'Social'},
  forum_debut:    {id:'forum_debut',   name:'Forum Debut',      icon:'✍️',rarity:'common',   color:'#1abc9c',desc:'Make your first forum post',           cat:'Social'},
  forum_star:     {id:'forum_star',    name:'Forum Star',       icon:'❤️',rarity:'rare',     color:'#e74c3c',desc:'Receive 25 likes on forum posts',      cat:'Social'},
  collector:      {id:'collector',     name:'Collector',        icon:'🎨',rarity:'uncommon', color:'#d4a017',desc:'Own 3 card backgrounds',                cat:'Shop'},
};

function computeEarnedAchievements(user, extra) {
  const e = [];
  if(user.levels_completed>=1)    e.push('first_level');
  if(user.levels_completed>=10)   e.push('explorer_10');
  if(user.levels_completed>=50)   e.push('veteran_50');
  if(user.levels_completed>=100)  e.push('centurion_100');
  if(user.levels_completed>=500)  e.push('grind_500');
  if(user.levels_completed>=1000) e.push('legend_1000');
  if(user.words_found_total>=100)   e.push('words_100');
  if(user.words_found_total>=500)   e.push('words_500');
  if(user.words_found_total>=2500)  e.push('words_2500');
  if(user.words_found_total>=10000) e.push('words_10k');
  if(user.bonus_found_total>=1)  e.push('bonus_first');
  if(user.bonus_found_total>=10) e.push('bonus_hunter');
  if(user.bonus_found_total>=50) e.push('bonus_master');
  if(user.no_hint_levels>=1)  e.push('no_hints_first');
  if(user.no_hint_levels>=10) e.push('no_hints_10');
  if(user.no_hint_levels>=50) e.push('no_hints_50');
  if(extra.speedster)           e.push('speedster');
  if(extra.friendCount>=1)      e.push('friend_first');
  if(extra.friendCount>=5)      e.push('social_5');
  if(extra.forumPostCount>=1)   e.push('forum_debut');
  if(extra.forumLikesReceived>=25) e.push('forum_star');
  if(extra.cardBgCount>=3)      e.push('collector');
  return [...new Set(e)];
}

async function checkAndAwardAchievements(userId, hints={}) {
  try {
    const user = await get('SELECT levels_completed,words_found_total,bonus_found_total,no_hint_levels FROM users WHERE id=?', [userId]);
    if(!user) return [];
    const friendCount = +(await get("SELECT COUNT(*) as c FROM friendships WHERE (user_id=? OR friend_id=?) AND status='accepted'", [userId,userId]))?.c || 0;
    const forumPostCount = +(await get('SELECT COUNT(*) as c FROM forum_posts WHERE author_id=?', [userId]))?.c || 0;
    const forumLikesReceived = +(await get(`SELECT COUNT(*) as c FROM forum_reactions r JOIN forum_posts p ON r.post_id=p.id WHERE p.author_id=? AND r.type='like'`, [userId]))?.c || 0;
    const cardBgCount = (+(await get('SELECT COUNT(*) as c FROM user_card_bgs WHERE user_id=?', [userId]))?.c || 0) + 1;
    const fastLevel = hints.timeTaken != null ? null : await get('SELECT MIN(time_taken) as t FROM level_completions WHERE user_id=?', [userId]);
    const speedster = hints.timeTaken != null ? hints.timeTaken <= 90 : (fastLevel?.t != null && fastLevel.t <= 90);
    const extra = { speedster, friendCount, forumPostCount, forumLikesReceived, cardBgCount };
    const earned = computeEarnedAchievements(user, extra);
    const existing = (await all('SELECT achievement_id FROM user_achievements WHERE user_id=?', [userId])).map(r=>r.achievement_id);
    const newOnes = earned.filter(a=>!existing.includes(a));
    for(const id of newOnes) {
      try { await run('INSERT INTO user_achievements (user_id,achievement_id) VALUES (?,?) ON CONFLICT DO NOTHING', [userId, id]); } catch {}
      const ach = ACHIEVEMENTS[id];
      if(ach) io.to(`u:${userId}`).emit('achievement:unlocked', ach);
    }
    return newOnes.map(id=>ACHIEVEMENTS[id]).filter(Boolean);
...
const EVENTS = {
  ...
  // ── Auto weather (mild) ───────────────────────────────────────────────────
  breeze:       {id:'breeze',      name:'Light Breeze',   icon:'🌬️',desc:'+12% XP',              xpMult:1.12,coinMult:1,    scoreMult:1,    duration:10,auto:true},
  warm_rain:    {id:'warm_rain',   name:'Warm Rain',      icon:'🌦️',desc:'+12% Coins',            xpMult:1,   coinMult:1.12, scoreMult:1,    duration:10,auto:true},
  clear_sky:    {id:'clear_sky',   name:'Clear Sky',      icon:'☀️',desc:'+15% Score',            xpMult:1,   coinMult:1,    scoreMult:1.15, duration:12,auto:true},
  cool_snap:    {id:'cool_snap',   name:'Cool Snap',      icon:'❄️',desc:'+8% XP, Coins & Score', xpMult:1.08,coinMult:1.08, scoreMult:1.08, duration:12,auto:true},
  ...
  // ── Owner weather (powerful) ──────────────────────────────────────────────
  thunderstorm: {id:'thunderstorm',name:'Thunderstorm',   icon:'⛈️',desc:'2x XP + 2x Score',     xpMult:2,   coinMult:1,    scoreMult:2,    duration:15,auto:false},
  blizzard:     {id:'blizzard',    name:'Blizzard',       icon:'🌨️',desc:'4x Coins',              xpMult:1,   coinMult:4,    scoreMult:1,    duration:8, auto:false},
  solar_flare:  {id:'solar_flare', name:'Solar Flare',    icon:'🌞',desc:'3x XP + 1.5x Score',   xpMult:3,   coinMult:1,    scoreMult:1.5,  duration:10,auto:false},
  golden_sunset:{id:'golden_sunset',name:'Golden Sunset', icon:'🌅',desc:'3x Coins + 1.5x Score', xpMult:1,   coinMult:3,    scoreMult:1.5,  duration:20,auto:false},
  supercell:    {id:'supercell',   name:'Supercell',      icon:'🌪️',desc:'2.5x Everything',       xpMult:2.5, coinMult:2.5,  scoreMult:2.5,  duration:12,auto:false},
};
...
app.post('/api/game/complete', auth, async (req, res) => {
  const { level, score, timeTaken, wordsFound, hintsUsed = 0, bonusFound = 0 } = req.body;
  ...
  try {
    ...
    const noHintIncrement = (hintsUsed === 0) ? 1 : 0;
    await run('UPDATE users SET xp=xp+?,coins=coins+?,levels_completed=levels_completed+1,words_found_total=words_found_total+?,bonus_found_total=bonus_found_total+?,no_hint_levels=no_hint_levels+? WHERE id=?',
      [xp, coins, wordsFound || 0, bonusFound || 0, noHintIncrement, uid]);
    const user = await get('SELECT id,username,xp,coins,levels_completed,rank_points,role,active_icon,active_title,active_card_bg FROM users WHERE id=?', [uid]);
    ...
    const newAchievements = await checkAndAwardAchievements(uid, { timeTaken });
    const eventsActive = Object.values(activeEvents).map(({event,endsAt})=>({...event,endsAt}));
    res.json({ xpGain: xp, coinGain: coins, user, newTitles: newIds.map(id => TITLES[id]), newAchievements, eventsActive, mults });
  } catch { res.status(500).json({ error: 'Server error' }); }
});
...
app.get('/api/game/leaderboard/:type', async (req, res) => {
  ...
  const rows = await all(`SELECT id,username,role,active_title,xp,coins,levels_completed,rank_points,wins,losses FROM users WHERE is_banned=0 ORDER BY ${col} DESC LIMIT 500`);
  res.json(rows.map((r,i) => ({ ...r, top500: i < 500, position: i+1 })));
});

async function isTop500(userId) {
  const row = await get(`SELECT COUNT(*) as c FROM users WHERE rank_points > (SELECT rank_points FROM users WHERE id=?) AND is_banned=0`, [userId]);
  return +row.c < 500;
}

app.get('/api/game/me', auth, async (req, res) => {
  const user = await get('SELECT id,username,email,role,is_banned,xp,coins,levels_completed,rank_points,wins,losses,active_title,active_icon,active_card_bg,bio,daily_streak,created_at FROM users WHERE id=?', [req.user.id]);
  if (user?.is_banned) return res.status(403).json({ error: 'Banned' });
  user.top500 = await isTop500(user.id);
  res.json(user);
});

app.patch('/api/game/bio', auth, async (req, res) => {
  const bio = (req.body.bio || '').trim().slice(0, 150);
  await run('UPDATE users SET bio=? WHERE id=?', [bio || null, req.user.id]);
  res.json({ success: true, bio: bio || null });
});

app.get('/api/game/profile/:id', auth, async (req, res) => {
  const user = await get('SELECT id,username,role,xp,coins,levels_completed,rank_points,wins,losses,active_title,active_icon,active_card_bg,bio,daily_streak,created_at FROM users WHERE id=?', [req.params.id]);
...
app.post('/api/friends/request/:fid', auth, async (req, res) => {
  ...
  const sender = await get('SELECT username FROM users WHERE id=?', [uid]);
  await notify(fid, 'friend_request', `${sender.username} sent you a friend request`);
  res.json({ success: true });
});

app.post('/api/friends/accept/:fid', auth, async (req, res) => {
  const fid = +req.params.fid, uid = req.user.id;
  await run("UPDATE friendships SET status='accepted' WHERE user_id=? AND friend_id=? AND status='pending'", [fid, uid]);
  const accepter = await get('SELECT username FROM users WHERE id=?', [uid]);
  await notify(fid, 'friend_accepted', `${accepter.username} accepted your friend request`);
  checkAndAwardAchievements(uid).catch(()=>{});
  checkAndAwardAchievements(fid).catch(()=>{});
  res.json({ success: true });
});
...
app.get('/api/notifications', auth, async (req, res) => {
  const rows = await all('SELECT * FROM notifications WHERE user_id=? ORDER BY created_at DESC LIMIT 30', [req.user.id]);
...
app.get('/api/admin/stats', ...adm, async (req, res) => {
  ...
  const [banned] = await all('SELECT COUNT(*) c FROM users WHERE is_banned=1');
  const [posts] = await all('SELECT COUNT(*) c FROM forum_posts');
  const [reps] = await all("SELECT COUNT(*) c FROM reports WHERE status='open'");
  const [achs] = await all('SELECT COUNT(*) c FROM user_achievements');
  ...
  const recentActivity = await all(`SELECT lc.completed_at, u.username, lc.level_number, lc.score FROM level_completions lc JOIN users u ON u.id=lc.user_id ORDER BY lc.completed_at DESC LIMIT 15`);
  res.json({ totalUsers: +u.c, totalLevels: +l.c, totalMatches: +m.c, totalMessages: +msg.c, bannedUsers: +banned.c, forumPosts: +posts.c, openReports: +reps.c, totalAchievements: +achs.c, onlineNow: online, recentUsers: recent, recentActivity });
});
...
app.patch('/api/admin/users/:id/coins', ...adm, requireRole('owner'), async (req, res) => {
...
app.patch('/api/admin/users/:id/xp', ...adm, requireRole('owner'), async (req, res) => {
  if (typeof req.body.amount !== 'number') return res.status(400).json({ error: 'Invalid' });
  await run('UPDATE users SET xp=MAX(0,xp+?) WHERE id=?', [req.body.amount, req.params.id]);
  res.json({ success: true });
});
app.patch('/api/admin/users/:id/rp', ...adm, requireRole('owner'), async (req, res) => {
  if (typeof req.body.amount !== 'number') return res.status(400).json({ error: 'Invalid' });
  await run('UPDATE users SET rank_points=MAX(0,rank_points+?) WHERE id=?', [req.body.amount, req.params.id]);
  res.json({ success: true });
});
app.post('/api/admin/users/:id/mute', ...adm, requireRole('mod'), async (req, res) => {
  const t = await get('SELECT role FROM users WHERE id=?', [req.params.id]);
  if (!t || t.role === 'owner') return res.status(403).json({ error: 'Cannot mute' });
  if (ROLE_LEVELS[t.role] >= ROLE_LEVELS[req.user.role]) return res.status(403).json({ error: 'Cannot mute equal/higher role' });
  const muted = req.body.muted ? 1 : 0;
  await run('UPDATE users SET is_muted=? WHERE id=?', [muted, req.params.id]);
  io.to(`u:${+req.params.id}`).emit('user:muted', { muted: !!muted });
  res.json({ success: true });
});
app.post('/api/admin/users/:id/kick', ...adm, requireRole('mod'), async (req, res) => {
  const t = await get('SELECT role FROM users WHERE id=?', [req.params.id]);
  if (!t || t.role === 'owner') return res.status(403).json({ error: 'Cannot kick' });
  if (ROLE_LEVELS[t.role] >= ROLE_LEVELS[req.user.role]) return res.status(403).json({ error: 'Cannot kick equal/higher role' });
  io.to(`u:${+req.params.id}`).emit('user:kicked', { reason: req.body.reason || 'Kicked by staff' });
  res.json({ success: true });
});
app.post('/api/admin/users/:id/reset-stats', ...adm, requireRole('owner'), async (req, res) => {
  const t = await get('SELECT role FROM users WHERE id=?', [req.params.id]);
  if (!t || t.role === 'owner') return res.status(403).json({ error: 'Cannot reset owner' });
  const { field } = req.body;
  if (field === 'coins') await run('UPDATE users SET coins=500 WHERE id=?', [req.params.id]);
  else if (field === 'xp') await run('UPDATE users SET xp=0,levels_completed=0 WHERE id=?', [req.params.id]);
  else if (field === 'rp') await run('UPDATE users SET rank_points=1000,wins=0,losses=0 WHERE id=?', [req.params.id]);
  else return res.status(400).json({ error: 'Invalid field' });
  res.json({ success: true });
});
app.get('/api/admin/activity', ...adm, async (req, res) => {
  const levels = await all(`SELECT 'level' as type, u.username, lc.level_number as detail, lc.score as score, lc.completed_at as time FROM level_completions lc JOIN users u ON u.id=lc.user_id ORDER BY lc.completed_at DESC LIMIT 30`);
  const matches = await all(`SELECT 'match' as type, u.username, rm.level_number as detail, rm.player1_score as score, rm.ended_at as time FROM ranked_matches rm JOIN users u ON u.id=rm.winner_id WHERE rm.status='completed' AND rm.ended_at IS NOT NULL ORDER BY rm.ended_at DESC LIMIT 20`);
  const combined = [...levels, ...matches].sort((a,b)=>new Date(b.time)-new Date(a.time)).slice(0,40);
  res.json(combined);
});
app.get('/api/admin/forum/recent', ...adm, async (req, res) => {
  const posts = await all(`SELECT fp.*,u.username as author_name FROM forum_posts fp LEFT JOIN users u ON u.id=fp.author_id ORDER BY fp.created_at DESC LIMIT 30`);
  res.json(posts);
});
...
async function notify(userId, type, message) {
  await run('INSERT INTO notifications (user_id,type,message) VALUES (?,?,?)', [userId, type, message]);
  const unread = await get('SELECT COUNT(*) as c FROM notifications WHERE user_id=? AND read=0', [userId]);
  io.to(`u:${userId}`).emit('notification', { type, message, unread: +unread.c });
}
...
io.on('connection', async socket => {
  ...
  const unreadNotifs = await get('SELECT COUNT(*) as c FROM notifications WHERE user_id=? AND read=0', [uid]);
  if (+unreadNotifs.c > 0) socket.emit('notifications:unread', { count: +unreadNotifs.c });
  ...
  socket.on('chat:message', async ({ friendId, content }) => {
    ...
    // Notify if recipient is not actively in chat with sender
    await notify(friendId, 'chat_message', `${socket.user.username}: ${content.trim().slice(0, 60)}`);
  });
...
const CARD_BACKGROUNDS = [
  {id:'default',      name:'Default',      price:0,     rarity:'common',    colors:['#06080e','#0d1b3e']},
  {id:'crimson',      name:'Crimson',      price:4000,  rarity:'common',    colors:['#1a0005','#4a0010','#1a0005']},
  {id:'ocean',        name:'Ocean',        price:4000,  rarity:'common',    colors:['#001a2c','#003a5c','#001a2c']},
  {id:'forest',       name:'Forest',       price:4000,  rarity:'common',    colors:['#001a0a','#003a15','#001a0a']},
  {id:'violet',       name:'Violet',       price:4000,  rarity:'common',    colors:['#0f001a','#2a0040','#0f001a']},
  {id:'sunset',       name:'Sunset',       price:8000,  rarity:'rare',      colors:['#1a0800','#3a1200','#1a0500']},
  {id:'galaxy',       name:'Galaxy',       price:12000, rarity:'rare',      colors:['#020008','#0d0030','#08001a']},
  {id:'aurora',       name:'Aurora',       price:18000, rarity:'epic',      colors:['#001a15','#0a2030','#15000a']},
  {id:'inferno',      name:'Inferno',      price:22000, rarity:'epic',      colors:['#1a0000','#3a0800','#500000']},
  {id:'gold',         name:'Gold',         price:35000, rarity:'epic',      colors:['#1a1000','#3a2500','#1a1000']},
  // ── Staff exclusive (role-gated, unpurchasable) ───────────────────────────
  {id:'staff_mod',    name:'Staff Blue',   price:0, rarity:'exclusive', colors:['#000d20','#001a50','#0050c8','#001a50'], requiredRole:'mod'},
  {id:'staff_admin',  name:'Enforcer',     price:0, rarity:'exclusive', colors:['#080015','#15003a','#3d00a0','#15003a'], requiredRole:'admin'},
  {id:'staff_manager',name:'Commander',    price:0, rarity:'exclusive', colors:['#150000','#3a0a00','#8c1a00','#c0392b','#ff4500'], requiredRole:'staff-manager'},
  {id:'sovereign',    name:'Sovereign',    price:0, rarity:'unique',    colors:['#050300','#1a1000','#4a3200','#9a7200','#d4a017','#ffd700','#fffae0'], requiredRole:'owner'},
];

app.get('/api/shop/card-bgs', auth, async (req, res) => {
  try {
    const purchased = await all('SELECT bg_id FROM user_card_bgs WHERE user_id=?', [req.user.id]);
    const purchasedSet = new Set(purchased.map(r => r.bg_id));
    const user = await get('SELECT coins, active_card_bg, role FROM users WHERE id=?', [req.user.id]);
    const userLevel = ROLE_LEVELS[user.role] || 0;
    res.json({ bgs: CARD_BACKGROUNDS.map(b => ({
      ...b,
      owned: b.requiredRole
        ? (ROLE_LEVELS[b.requiredRole] || 0) <= userLevel
        : (b.price === 0 || purchasedSet.has(b.id)),
      eligible: b.requiredRole ? (ROLE_LEVELS[b.requiredRole] || 0) <= userLevel : true,
    })), coins: user.coins, activeBg: user.active_card_bg || 'default' });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/shop/card-bgs/:id/buy', auth, async (req, res) => {
  try {
    const bg = CARD_BACKGROUNDS.find(b => b.id === req.params.id);
    if (!bg || bg.price === 0) return res.status(400).json({ error: 'Invalid' });
    const existing = await get('SELECT id FROM user_card_bgs WHERE user_id=? AND bg_id=?', [req.user.id, bg.id]);
    if (existing) return res.status(400).json({ error: 'Already owned' });
    const user = await get('SELECT coins FROM users WHERE id=?', [req.user.id]);
    if (user.coins < bg.price) return res.status(400).json({ error: 'Not enough coins' });
    await run('UPDATE users SET coins=coins-? WHERE id=?', [bg.price, req.user.id]);
    await run('INSERT INTO user_card_bgs (user_id,bg_id) VALUES (?,?)', [req.user.id, bg.id]);
    const updated = await get('SELECT coins FROM users WHERE id=?', [req.user.id]);
    checkAndAwardAchievements(req.user.id).catch(()=>{});
    res.json({ success: true, coins: updated.coins });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/shop/card-bgs/:id/equip', auth, async (req, res) => {
  try {
    ...
    if (bg.requiredRole) {
      ...
      if ((ROLE_LEVELS[user.role] || 0) < (ROLE_LEVELS[bg.requiredRole] || 99))
        return res.status(403).json({ error: `Requires ${bg.requiredRole} role` });
    } else if (bg.price > 0) {
      const owned = await get('SELECT id FROM user_card_bgs WHERE user_id=? AND bg_id=?', [req.user.id, bgId]);
      if (!owned) return res.status(403).json({ error: 'Not owned' });
    }
    await run('UPDATE users SET active_card_bg=? WHERE id=?', [bgId, req.user.id]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/ratings', async (req, res) => {
  try {
    const summary = await get("SELECT ROUND(AVG(rating)::numeric,1) as avg, COUNT(*) as total FROM game_ratings");
    const reviews = await all(`SELECT gr.rating, gr.comment, gr.updated_at, u.username FROM game_ratings gr
      JOIN users u ON u.id=gr.user_id WHERE gr.comment IS NOT NULL AND gr.comment != ''
      ORDER BY gr.updated_at DESC LIMIT 20`);
    res.json({ avg: +summary.avg || 0, total: +summary.total || 0, reviews });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/ratings/mine', auth, async (req, res) => {
  try {
    const r = await get('SELECT rating, comment FROM game_ratings WHERE user_id=?', [req.user.id]);
    res.json(r || null);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/ratings', auth, async (req, res) => {
  try {
    ...
    const existing = await get('SELECT id FROM game_ratings WHERE user_id=?', [req.user.id]);
    if (existing) {
      await run('UPDATE game_ratings SET rating=?, comment=?, updated_at=NOW() WHERE user_id=?', [rating, comment||null, req.user.id]);
    } else {
      await run('INSERT INTO game_ratings (user_id,rating,comment) VALUES (?,?,?)', [req.user.id, rating, comment||null]);
    }
    const summary = await get("SELECT ROUND(AVG(rating)::numeric,1) as avg, COUNT(*) as total FROM game_ratings");
    res.json({ avg: +summary.avg, total: +summary.total });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/forum/posts', auth, async (req, res) => {
  try {
    ...
    const rows = await all(`
      SELECT fp.id, fp.title, fp.content, fp.section, fp.pinned, fp.created_at, fp.media,
        u.username as author_name, u.role as author_role,
        (SELECT COUNT(*) FROM forum_replies fr WHERE fr.post_id=fp.id) as reply_count,
        (SELECT COUNT(*) FROM forum_reactions r WHERE r.post_id=fp.id AND r.type='like') as likes,
        (SELECT COUNT(*) FROM forum_reactions r WHERE r.post_id=fp.id AND r.type='dislike') as dislikes,
        (SELECT type FROM forum_reactions r WHERE r.post_id=fp.id AND r.user_id=?) as user_reaction
      FROM forum_posts fp LEFT JOIN users u ON u.id=fp.author_id
      WHERE fp.section=? ORDER BY fp.pinned DESC, fp.created_at DESC`, [req.user.id, section]);
    res.json(rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/forum/posts', auth, attachRole, async (req, res) => {
  try {
    ...
    const muted = await get('SELECT is_muted FROM users WHERE id=?', [req.user.id]);
    if (muted?.is_muted) return res.status(403).json({ error: 'You are muted and cannot post.' });
...
app.get('/api/forum/posts/:id', auth, async (req, res) => {
  try {
    const post = await get(`SELECT fp.*,u.username as author_name,u.role as author_role,
      (SELECT COUNT(*) FROM forum_reactions r WHERE r.post_id=fp.id AND r.type='like') as likes,
      (SELECT COUNT(*) FROM forum_reactions r WHERE r.post_id=fp.id AND r.type='dislike') as dislikes,
      (SELECT type FROM forum_reactions r WHERE r.post_id=fp.id AND r.user_id=?) as user_reaction
      FROM forum_posts fp LEFT JOIN users u ON u.id=fp.author_id WHERE fp.id=?`, [req.user.id, +req.params.id]);
    if (!post) return res.status(404).json({ error: 'Post not found' });
    const replies = await all(`SELECT fr.*,u.username as author_name,u.role as author_role,
      (SELECT COUNT(*) FROM forum_reactions r WHERE r.reply_id=fr.id AND r.type='like') as likes,
      (SELECT COUNT(*) FROM forum_reactions r WHERE r.reply_id=fr.id AND r.type='dislike') as dislikes,
      (SELECT type FROM forum_reactions r WHERE r.reply_id=fr.id AND r.user_id=?) as user_reaction
      FROM forum_replies fr LEFT JOIN users u ON u.id=fr.author_id WHERE fr.post_id=? ORDER BY fr.created_at ASC`, [req.user.id, post.id]);
...
app.post('/api/forum/posts/:id/react', auth, async (req, res) => {
  try {
    const { type } = req.body;
    if (!['like','dislike'].includes(type)) return res.status(400).json({ error: 'Invalid type' });
    const pid = +req.params.id, uid = req.user.id;
    const existing = await get('SELECT * FROM forum_reactions WHERE user_id=? AND post_id=? AND reply_id IS NULL', [uid, pid]);
    if (existing?.type === type) await run('DELETE FROM forum_reactions WHERE user_id=? AND post_id=? AND reply_id IS NULL', [uid, pid]);
    else if (existing) await run('UPDATE forum_reactions SET type=? WHERE user_id=? AND post_id=? AND reply_id IS NULL', [type, uid, pid]);
    else await run('INSERT INTO forum_reactions (user_id,post_id,type) VALUES (?,?,?)', [uid, pid, type]);
    const likes = await get("SELECT COUNT(*) as c FROM forum_reactions WHERE post_id=? AND type='like'", [pid]);
    const dislikes = await get("SELECT COUNT(*) as c FROM forum_reactions WHERE post_id=? AND type='dislike'", [pid]);
    const ur = await get('SELECT type FROM forum_reactions WHERE user_id=? AND post_id=? AND reply_id IS NULL', [uid, pid]);
    // Check forum_star for post owner
    const postOwner = await get('SELECT author_id FROM forum_posts WHERE id=?', [pid]);
    if(postOwner?.author_id) checkAndAwardAchievements(postOwner.author_id).catch(()=>{});
    res.json({ likes: +likes.c, dislikes: +dislikes.c, userReaction: ur?.type || null });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/forum/replies/:id/react', auth, async (req, res) => {
  try {
    const { type } = req.body;
    if (!['like','dislike'].includes(type)) return res.status(400).json({ error: 'Invalid type' });
    const rid = +req.params.id, uid = req.user.id;
    const existing = await get('SELECT * FROM forum_reactions WHERE user_id=? AND reply_id=?', [uid, rid]);
    if (existing?.type === type) await run('DELETE FROM forum_reactions WHERE user_id=? AND reply_id=?', [uid, rid]);
    else if (existing) await run('UPDATE forum_reactions SET type=? WHERE user_id=? AND reply_id=?', [type, uid, rid]);
    else await run('INSERT INTO forum_reactions (user_id,reply_id,type) VALUES (?,?,?)', [uid, rid, type]);
    const likes = await get("SELECT COUNT(*) as c FROM forum_reactions WHERE reply_id=? AND type='like'", [rid]);
    const dislikes = await get("SELECT COUNT(*) as c FROM forum_reactions WHERE reply_id=? AND type='dislike'", [rid]);
    const ur = await get('SELECT type FROM forum_reactions WHERE user_id=? AND reply_id=?', [uid, rid]);
    res.json({ likes: +likes.c, dislikes: +dislikes.c, userReaction: ur?.type || null });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// ── ACHIEVEMENTS API ──────────────────────────────────────────────────────────
app.get('/api/achievements/all', (_req, res) => res.json(Object.values(ACHIEVEMENTS)));

app.get('/api/achievements/mine', auth, async (req, res) => {
  try {
    const rows = await all('SELECT achievement_id, unlocked_at FROM user_achievements WHERE user_id=? ORDER BY unlocked_at DESC', [req.user.id]);
    res.json(rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/daily/status', auth, async (req, res) => {
  try {
    const today = todayStr();
    const done = await get('SELECT id FROM daily_challenge_completions WHERE user_id=? AND date_str=?', [req.user.id, today]);
    const user = await get('SELECT daily_streak FROM users WHERE id=?', [req.user.id]);
    const now = new Date();
    const nextReset = new Date(today); nextReset.setDate(nextReset.getDate() + 1);
    res.json({ completed: !!done, streak: user?.daily_streak || 0, nextResetMs: nextReset - now });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/daily/complete', auth, async (req, res) => {
  try {
    const today = todayStr();
    const uid = req.user.id;
    const existing = await get('SELECT id FROM daily_challenge_completions WHERE user_id=? AND date_str=?', [uid, today]);
    if (existing) return res.status(409).json({ error: 'Already completed today' });
    const { score = 0, wordsFound = 0 } = req.body;
    // Calculate streak
    const user = await get('SELECT daily_streak, last_daily_date FROM users WHERE id=?', [uid]);
    const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);
    const yStr = yesterday.toISOString().slice(0, 10);
    const newStreak = user.last_daily_date === yStr ? (user.daily_streak || 0) + 1 : 1;
    // Base reward + streak bonus (capped at 30 days)
    const baseReward = 300; const baseXp = 150;
    const streakBonus = Math.min(newStreak * 50, 1500);
    const totalCoins = baseReward + streakBonus;
    await run('INSERT INTO daily_challenge_completions (user_id,date_str,score,words_found) VALUES (?,?,?,?)', [uid, today, score, wordsFound]);
    await run('UPDATE users SET daily_streak=?,last_daily_date=?,coins=coins+?,xp=xp+?,levels_completed=levels_completed+1 WHERE id=?', [newStreak, today, totalCoins, baseXp, uid]);
    // Streak milestones
    const milestones = {3:'🔥 3-Day Streak!', 7:'⚡ 7-Day Streak!', 14:'💎 2-Week Streak!', 30:'👑 30-Day Legend!'};
    const milestone = milestones[newStreak] || null;
    const newAchievements = await checkAndAwardAchievements(uid).catch(() => []);
    const updatedUser = await get('SELECT id,username,xp,coins,levels_completed,rank_points,role,active_icon,active_title,active_card_bg FROM users WHERE id=?', [uid]);
    res.json({ coins: totalCoins, xp: baseXp, streak: newStreak, streakBonus, milestone, newAchievements, user: updatedUser });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/achievements/user/:id', async (req, res) => {
  try {
    const rows = await all('SELECT achievement_id, unlocked_at FROM user_achievements WHERE user_id=? ORDER BY unlocked_at DESC', [req.params.id]);
    res.json(rows);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
