...
async function initDb() {
  ...
  if (!owner) {
    ...
    await run("INSERT INTO users (username,email,password_hash,role,coins,xp) VALUES (?,?,?,?,?,?)",
      ['AMGProdZ27','owner@crosswordlegends.gg',hash,'owner',999999,999999]);
...
const ROLE_LEVELS = { user: 0, mod: 1, admin: 2, owner: 3 };
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    if (await get('SELECT id FROM users WHERE username=? OR email=?', [username, email]))
      return res.status(409).json({ error: 'Username or email taken' });
    const hash = await bcrypt.hash(password, 10);
    const { lastInsertRowid: id } = await run('INSERT INTO users (username,email,password_hash) VALUES (?,?,?)', [username, email, hash]);
...
app.post('/api/game/complete', auth, async (req, res) => {
  ...
  const xp = Math.floor(score * 1.5 + level * 10), coins = Math.floor(score * 0.5 + level * 5);
  try {
    ...
    res.json({ xpGain: xp, coinGain: coins, user, newTitles: newIds.map(id => TITLES[id]) });
  } catch { res.status(500).json({ error: 'Server error' }); }
});
