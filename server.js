...
const db = process.env.DATABASE_URL
  ...
  : createClient({ url: `file:${path.join(__dirname, 'crossword.db')}` });
...
async function initDb() {
  await db.executeMultiple(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      active_title TEXT DEFAULT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP
    );
    CREATE TABLE IF NOT EXISTS level_completions (
      ...
      words_found INTEGER DEFAULT 0, completed_at DATETIME DEFAULT CURRENT_TIMESTAMP
    );
    ...
    CREATE TABLE IF NOT EXISTS messages (
      id INTEGER PRIMARY KEY AUTOINCREMENT, sender_id INTEGER REFERENCES users(id),
    ...
    CREATE TABLE IF NOT EXISTS ranked_matches (
      id INTEGER PRIMARY KEY AUTOINCREMENT, player1_id INTEGER, player2_id INTEGER,
      winner_id INTEGER, level_number INTEGER DEFAULT 1,
      player1_score INTEGER DEFAULT 0, player2_score INTEGER DEFAULT 0,
      status TEXT DEFAULT 'waiting', created_at DATETIME DEFAULT CURRENT_TIMESTAMP, ended_at DATETIME
    );
    CREATE TABLE IF NOT EXISTS user_titles (
      id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      title_id TEXT NOT NULL, unlocked_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id, title_id)
    );
  `);
  try { await db.execute("ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'user'"); } catch {}
  try { await db.execute("ALTER TABLE users ADD COLUMN is_banned INTEGER DEFAULT 0"); } catch {}
  try { await db.execute("ALTER TABLE users ADD COLUMN active_title TEXT DEFAULT NULL"); } catch {}
  await db.execute(`CREATE INDEX IF NOT EXISTS idx_ut ON user_titles(user_id)`);
...
async function getLbPos(userId) {
  const lbs = {
    rank: await all('SELECT id FROM users WHERE is_banned=0 ORDER BY rank_points DESC LIMIT 50'),
    xp: await all('SELECT id FROM users WHERE is_banned=0 ORDER BY xp DESC LIMIT 50'),
    coins: await all('SELECT id FROM users WHERE is_banned=0 ORDER BY coins DESC LIMIT 50'),
    levels: await all('SELECT id FROM users WHERE is_banned=0 ORDER BY levels_completed DESC LIMIT 50'),
  };
  const pos = {};
...
async function awardTitles(userId) {
  ...
  for (const id of newOnes) try { await run('INSERT OR IGNORE INTO user_titles (user_id,title_id) VALUES (?,?)', [userId, id]); } catch {}
  return newOnes;
}
...
function startEvent(eventId, source='auto') {
  const e = EVENTS[eventId];
  if (!e) return null;
  if (activeEvents[eventId]) clearTimeout(activeEvents[eventId].timeout);
  const endsAt = Date.now() + e.duration * 60 * 1000;
  activeEvents[eventId] = {
    event: e, endsAt,
    timeout: setTimeout(() => { delete activeEvents[eventId]; io.emit('event:ended', {id:eventId}); }, e.duration * 60 * 1000)
  };
  io.emit('event:started', {event:e, endsAt, source});
  return e;
}
...
setInterval(() => {
  if (Math.random() < 0.4) {
    const pool = Object.keys(EVENTS).filter(k => EVENTS[k].auto);
    startEvent(pool[Math.floor(Math.random()*pool.length)], 'auto');
  }
}, 25 * 60 * 1000);

// ── AUTH MIDDLEWARE ───────────────────────────────────────────────────────────
const JWT_SECRET = process.env.JWT_SECRET || 'crossword_secret_2024';
const ROLE_LEVELS = { user: 0, mod: 1, admin: 2, 'co-owner': 3, owner: 4 };
...
app.post('/api/game/complete', auth, async (req, res) => {
  ...
  const xp = Math.floor((score * 1.5 + level * 10) * mults.xp);
  const coins = Math.floor((score * 0.5 + level * 5) * mults.coin);
  try {
    ...
    const user = await get('SELECT id,username,xp,coins,levels_completed,rank_points FROM users WHERE id=?', [uid]);
...
app.get('/api/friends', auth, async (req, res) => {
  const uid = req.user.id;
  res.json(await all(`SELECT u.id,u.username,u.xp,u.rank_points,u.levels_completed,f.status,CASE WHEN f.user_id=? THEN 'sent' ELSE 'received' END as direction FROM friendships f JOIN users u ON (u.id=CASE WHEN f.user_id=? THEN f.friend_id ELSE f.user_id END) WHERE f.user_id=? OR f.friend_id=?`, [uid, uid, uid, uid]));
});
...
app.get('/api/admin/users', ...adm, async (req, res) => res.json(await all('SELECT id,username,email,role,is_banned,xp,coins,levels_completed,rank_points,wins,losses,created_at FROM users ORDER BY created_at DESC')));
...
app.patch('/api/admin/users/:id/role', ...adm, requireRole('admin'), async (req, res) => {
  const { role } = req.body;
  const validRoles = ['user','mod','admin','co-owner'];
  if (!validRoles.includes(role)) return res.status(400).json({ error: 'Invalid role' });
  if (role === 'co-owner' && req.user.role !== 'owner') return res.status(403).json({ error: 'Only owner can assign co-owner' });
...
app.patch('/api/admin/users/:id/ban', ...adm, async (req, res) => {
  const t = await get('SELECT * FROM users WHERE id=?', [req.params.id]);
  if (!t || t.role==='owner' || t.role==='co-owner') return res.status(403).json({ error: 'Cannot ban' });
  if (ROLE_LEVELS[t.role] >= ROLE_LEVELS[req.user.role]) return res.status(403).json({ error: 'Cannot ban equal/higher role' });
  await run('UPDATE users SET is_banned=? WHERE id=?', [req.body.banned?1:0, req.params.id]);
  res.json({ success: true });
});
...
app.post('/api/admin/event', auth, attachRole, requireRole('owner'), (req, res) => {
  ...
  const e = startEvent(eventId, req.user.username);
  res.json({ success: true, event: e });
});
app.get('/api/admin/events_list', auth, attachRole, requireRole('co-owner'), (req, res) => {
  res.json(Object.values(EVENTS));
});

// Co-owner request system
app.post('/api/admin/request', auth, attachRole, requireRole('co-owner'), (req, res) => {
...
app.post('/api/admin/request/:id/approve', auth, attachRole, requireRole('owner'), (req, res) => {
  ...
  if (r.type === 'announce') {
    ...
    startEvent(r.data.eventId, r.from);
  }
...
io.on('connection', async socket => {
  ...
  socket.on('ranked:join_queue', async () => {
    if (waiting.size > 0) {
      ...
      matches.set(mid, { id: mid, p1: wid, p2: uid, s1: 0, s2: 0, w1: [], w2: [], level, status: 'in_progress' });
  ...
  socket.on('disconnect', () => waiting.delete(uid));
});
...
async function endMatch(mid) {
  ...
  io.to(`m:${mid}`).emit('ranked:match_ended', { winnerId: win, p1Score: m.s1, p2Score: m.s2, rpGain: 25, rpLoss: 15 });
...
initDb().then(() => server.listen(PORT, () => console.log(`Running on port ${PORT}`))).catch(e => { console.error(e); process.exit(1); });
