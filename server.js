...
const ROLE_LEVELS = { user: 0, mod: 1, admin: 2, 'staff-manager': 3, 'co-owner': 3, owner: 4 };
...
app.patch('/api/admin/users/:id/ban', ...adm, async (req, res) => {
  ...
  if (['staff-manager','co-owner'].includes(t.role) && req.user.role!=='owner') return res.status(403).json({ error: 'Only owner can ban Staff Manager' });
...
app.post('/api/staff/action-request', auth, attachRole, requireRole('staff-manager'), async (req, res) => {
  ...
  if (!['warn','fire','timeout'].includes(action)) return res.status(400).json({ error: 'Invalid action' });
  if (!reason?.trim()) return res.status(400).json({ error: 'Reason required' });
  const target = await get('SELECT id,username,role FROM users WHERE id=?', [+targetId]);
  if (!target) return res.status(404).json({ error: 'User not found' });
  if (!['mod','admin'].includes(target.role)) return res.status(403).json({ error: 'Can only act on mods and admins' });
  const r = await run(
...
app.post('/api/staff/action-requests/:id/approve', auth, attachRole, requireRole('owner'), async (req, res) => {
  ...
  if (r.action === 'warn') {
    ...
    await run("UPDATE users SET timeout_until = NOW() + INTERVAL '1 hour' * ? WHERE id=?", [hrs, r.target_id]);
  }
  const target = await get('SELECT username FROM users WHERE id=?', [r.target_id]);
  const actionLabel = r.action==='warn'?'warned':r.action==='fire'?'fired (demoted to user)':`given a ${r.timeout_hours||24}h timeout`;
...
io.on('connection', async socket => {
  ...
  socket.on('disconnect', async () => {
    waiting.delete(uid); onlineUsers.delete(uid);
...
async function endMatch(mid) {
  ...
  // Emit result immediately so clients never freeze
  io.to(`m:${mid}`).emit('ranked:match_ended', { winnerId: win, p1Score: m.s1, p2Score: m.s2, rpGain: 25, rpLoss: 15 });
  // Save to DB in background — don't let failures block the event
  try {
